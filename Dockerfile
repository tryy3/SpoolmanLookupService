...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -ldflags="-w -s" -o /app/spoolman-lookup-service .

# Runtime stage - using scratch for minimal image size
FROM scratch

# Copy the binary
COPY --from=builder /app/spoolman-lookup-service /spoolman-lookup-service

# Run as non-root (using numeric UID for scratch image)
USER 65534:65534

ENTRYPOINT ["/spoolman-lookup-service"]
